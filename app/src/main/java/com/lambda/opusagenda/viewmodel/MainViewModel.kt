    ...
    fun addTask(
        ...
        launchDatabaseAction(successMessage = appContext.getString(R.string.task_saved)) {
            val entity = TaskEntity(
                ...
                description = if (isCategory) null else description?.trim()?.ifBlank { null },
                link = if (isCategory) null else link?.trim()?.ifBlank { null },
                attachmentUri = if (isCategory) null else attachmentUri?.trim()?.ifBlank { null },
                attachmentName = if (isCategory) null else attachmentName?.trim()?.ifBlank { null },
                attachmentMimeType = if (isCategory) null else attachmentMimeType?.trim()?.ifBlank { null },
